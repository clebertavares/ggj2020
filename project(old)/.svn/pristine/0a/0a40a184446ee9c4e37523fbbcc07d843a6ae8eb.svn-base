	...
	public class Inventory : MonoBehaviour 
	{
		...
		public void AddExtraSlots(int amount){
			...
			for(int i = 0; i < amount; i++){
				...
				// Create the Slot.
				slots.Add (Instantiate(inventorySlot));
				// Assign the Slot a slot number by taking the default and adding how many extra we have + 1.
				slots [i].GetComponent<Item_Slot> ().slotNumber = defaultSlotAmount + extraInventorySlots + 1;
				// Set the Slot parent to the Slot Panel.
				slots [i].transform.SetParent (slotPanel.transform);
				// Set the scaling to 1.
				slots [i].transform.localScale = Vector2.one;
